async function runTestSuite() {
  ...
  // 2. Dashboard KPIs
  const dashRes = await fetch("http://localhost:4000/api/dashboard", { headers });
  ...
  const wfRes = await fetch("http://localhost:4000/api/workflows", { headers });
  const wf = await wfRes.json();
  console.log("✓ 3. Live Workflow Queues:", `${wf.totalCount} active items across 8 domains`);

  // 4. Multi-Stream Predictions
  const predRes = await fetch("http://localhost:4000/api/predictions", { headers });
  ...
  // 5. Anomalies
  const anomRes = await fetch("http://localhost:4000/api/anomalies", { headers });
  ...
  // 6. Tasks
  const taskRes = await fetch("http://localhost:4000/api/tasks", { headers });
  ...
  const scenRes = await fetch("http://localhost:4000/api/scenarios/simulate", {
    method: "POST",
    headers,
